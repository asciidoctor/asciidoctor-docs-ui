...
// Project URLs:
:project-repo-name: c2pa-org/c2pa-docs-ui
...
This project provides the UI for the C2PA docs site, which is based on the {url-asciidoctor}[AsciiDoctor documentation site].
...
. Update the README to reference the URL of the latest bundle and commit that update to the repository.
...
In addition to the <<Prerequisites>> covered above, you'll need a personal access token for the automated GitHub account, c2pa-docbot, so it has permission to make changes to the repository on GitHub.
The c2pa-docbot account will need at least write access to the {url-project} repository, though admin access is recommended.

Start by creating a https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/[personal access token] for the c2pa-docbot user.
...
No other scopes are required (as long as the c2pa-docbot user has write access to the repository).
...
Copyright (C) {https://c2pa.org}[C2PA] 2021-present.
This includes any CSS that provides colors or iconography that depict the C2PA brand.
